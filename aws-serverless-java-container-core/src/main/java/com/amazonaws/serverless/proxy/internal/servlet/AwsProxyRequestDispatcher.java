...
public class AwsProxyRequestDispatcher implements RequestDispatcher {
    ...
    /**
     * Sets the destination path in the given request. Uses the <code>APIGatewayProxyRequestEvent.setPath</code> method which
     ...
     *  attribute or the value for the attribute is not of the correct type: <code>APIGatewayProxyRequestEvent</code>.
     */
    void setRequestPath(ServletRequest req, final String destinationPath) {
        ...
        if (req.getAttribute(API_GATEWAY_EVENT_PROPERTY) != null && req.getAttribute(API_GATEWAY_EVENT_PROPERTY) instanceof APIGatewayProxyRequestEvent) {
            ((APIGatewayProxyRequestEvent)req.getAttribute(API_GATEWAY_EVENT_PROPERTY)).setPath(dispatchTo);
            return;
        }
