...
public class AwsAlbHttpServletResponseWriterTest {
    ...
    void writeResponse_returnsValidResponse() throws InvalidRequestEventException, InvalidResponseObjectException {
        ...
        ApplicationLoadBalancerResponseEvent res3 = responseWriter.writeResponse(response, new MockLambdaContext());
        assertTrue(res3.getHeaders().isEmpty());

        response.setContentType("application/octet-stream");
        ApplicationLoadBalancerResponseEvent res2 = responseWriter.writeResponse(response, new MockLambdaContext());
        assertTrue(res2.getIsBase64Encoded());

        response.setHeader("Connection", "Keep-Alive");
        response.setContentType("application/octet-stream;");
        response.setStatus(200);
        ApplicationLoadBalancerResponseEvent res1 = responseWriter.writeResponse(response, new MockLambdaContext());
        assertNotNull(res1);
